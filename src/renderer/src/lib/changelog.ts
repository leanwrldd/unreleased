...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-06-25',
    changes: [
      { type: 'fix',     text: 'Random mode now respects active category/era filter when prefetching tracks' },
      { type: 'fix',     text: 'Tracker search results now persist when switching tabs (not just the input text)' },
      ...
      { type: 'fix',     text: 'Shuffle no longer re-randomizes on each skip; advances linearly through the pre-shuffled queue' },
      { type: 'fix',     text: 'Local files grid view toggle now persists when navigating away and back' },
      { type: 'fix',     text: 'Window close/minimize/maximize buttons now respond correctly' },
      { type: 'fix',     text: 'App updates no longer log the user out (only auth errors clear the session)' },
      { type: 'new',     text: 'Local playlist cards support right-click context menu and visible mobile menu button' },
      { type: 'new',     text: 'Metadata editor redesigned with Lyrics / Synced Lyrics tabs' },
      { type: 'improve', text: 'Radio mode renamed to Random mode throughout the UI' },
    ],
  },
  {
    version: '1.8.7',
